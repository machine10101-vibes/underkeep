...
export const TORCH_LIGHT = { intensity: 7.6, distance: 26, decay: 1.15 } as const;
...
export function makeTorchMesh(withLight = true): THREE.Group {
  ...
  const glow = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: 0xff9940, transparent: true, opacity: 0.78 })
  );
...
export function tileMaterial(kind: TileKind, fortified: boolean, _room: RoomType): THREE.MeshStandardMaterial {
  ...
  switch (kind) {
    ...
    case TileKind.Gold:
      return texturedMat('gold-v6', goldVeinTex(), {
        metalness: 0.28,
        roughness: 0.72,
        emissive: 0x6a3a08,
